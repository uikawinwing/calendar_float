...
export function 构建静默扫描输出(): 日历运行时内容节点['输出'] {
  return {
    模式: 'silent_scan',
    位置: 'none',
    深度: 0,
...
export function 构建可见注入输出(depth: number): 日历运行时内容节点['输出'] {
  return {
    模式: 'injectprompt',
    位置: 'in_chat',
    深度: depth,
...
export function 规范化提醒默认值(value: unknown): 日历运行时提醒默认值 {
...
export function 规范化书籍默认值(value: unknown): 日历运行时书籍默认值 {
...
export function 规范化默认设置(value: unknown): 日历运行时默认设置 | undefined {
  if (!是否对象(value)) {
    return undefined;
  }
