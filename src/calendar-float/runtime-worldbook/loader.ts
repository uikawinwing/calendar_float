...
import {
  type CalendarWorldbookReference,
  ...
  type CalendarRuntimeBookEntry,
  type CalendarRuntimeBookDefaults,
  type CalendarRuntimeContentNode,
  type CalendarRuntimeVariableCondition,
  type CalendarRuntimeReminderDefaults,
  type CalendarRuntimeTextLibrary,
  type CalendarRuntimeIndex,
  type CalendarRuntimeFestivalStageEntry,
  type CalendarRuntimeFestivalEntry,
  type CalendarRuntimeTriggerMap,
  type CalendarRuntimeLogicCondition,
  type CalendarRuntimeDefaults,
} from './types';
...
function 提取文本库映射(data: unknown): CalendarRuntimeTextLibrary {
  if (!是否对象(data)) {
    return {};
  }
...
function 补全内置节庆阶段列表(
  stages: CalendarRuntimeFestivalStageEntry[],
  festivalId: string,
  festivalName: string,
  festivalItem: Record<string, unknown>,
  reminderDefaults: CalendarRuntimeReminderDefaults,
