...
export function formatCalendarMonthTitle(year: number, month: number, alias?: string, eraName?: string): string {
  const normalizedYear = Number(year);
  const normalizedMonth = normalizeMonth(month);
  const aliasText = String(alias || '').trim();
  const eraText = String(eraName || '').trim();
  const yearText = `${eraText}${normalizedYear}年`;
  const monthText = Number.isFinite(normalizedMonth) ? `${normalizedMonth}月` : '月份未知';
  return aliasText ? `${yearText}·${monthText}（${aliasText}）` : `${yearText}·${monthText}`;
}
