...
function 规范化日期条件(value: unknown): CalendarRuntimeDateWindowCondition | null {
  if (!是否对象(value)) {
    return null;
  }
  ...
  const output: CalendarRuntimeDateWindowCondition = {};
  if (路径) {
    output.路径 = 路径;
  }
  if (开始) {
...
function 规范化变量条件(value: unknown): CalendarRuntimeVariableCondition | null {
  if (!是否对象(value)) {
    return null;
  }
  ...
  const output: CalendarRuntimeVariableCondition = { 路径 };
...
function 规范化原子条件(value: unknown): CalendarRuntimeAtomicCondition | null {
  if (!是否对象(value)) {
    return null;
  }
  ...
  const output: CalendarRuntimeAtomicCondition = {};
  if (来源) {
    output.来源 = 来源 as CalendarRuntimeAtomicCondition['来源'];
  }
  if (路径) {
    output.路径 = 路径;
  }
...
function 规范化逻辑条件(value: unknown): CalendarRuntimeLogicCondition | null {
  if (!是否对象(value)) {
    return null;
  }

  const output: CalendarRuntimeLogicCondition = {};
  ...
  for (const [canonical, aliases] of [
    ...
    const items: CalendarRuntimeLogicCondition[] = [];
...
export function 规范化触发映射(value: unknown): CalendarRuntimeTriggerMap | null {
  if (!是否对象(value)) {
    return null;
  }
  ...
  if (Array.isArray(rawVarConditions)) {
    const conditions: CalendarRuntimeVariableCondition[] = [];
...
function 构建触发逻辑节点(trigger: CalendarRuntimeTriggerMap | null | undefined): CalendarRuntimeLogicCondition | null {
  if (!trigger || trigger.启用 === false) {
    return null;
  }

  const 节点列表: CalendarRuntimeLogicCondition[] = [];
  if (trigger.完整逻辑) {
...
export function 合并触发映射(
  logic: '全部满足' | '任一满足',
  triggers: Array<CalendarRuntimeTriggerMap | null | undefined>,
): CalendarRuntimeTriggerMap | null {
  const nodes = triggers
    .map(item => 构建触发逻辑节点(item))
    .filter((item): item is CalendarRuntimeLogicCondition => Boolean(item));
  if (nodes.length === 0) {
    return null;
  }
  if (nodes.length === 1) {
    const matchedTrigger = triggers.find(
      (item): item is CalendarRuntimeTriggerMap => Boolean(item) && Boolean(构建触发逻辑节点(item)),
    );
