...
const targetEntry = normalize_entry_path(String(process.env.TAVERN_HELPER_ENTRY ?? '').trim());
const skipSchemaDump = process.env.TAVERN_HELPER_SKIP_DUMP === '1';
const skipTavernSync = process.env.TAVERN_HELPER_SKIP_SYNC === '1';

const config: Config = {
  port: 6621,
  entries: glob_script_files()
    .filter(file => !targetEntry || normalize_entry_path(file) === targetEntry)
    .map(parse_entry),
};

if (targetEntry && config.entries.length === 0) {
